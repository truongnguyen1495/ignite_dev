...
import { createMyWhiteboardAction, renameMyWhiteboardAction, deleteMyWhiteboardAction } from "./actions";

// Student-side twin of src/app/admin/whiteboards/whiteboard-list.tsx —
// separate file rather than a shared/prop-driven component (matches this
// app's convention of never cross-importing between the admin and
// dashboard route trees), plus one real UI difference: a board can be
// "của bạn" (owned) or "được chia sẻ" (shared to this student by someone
// else), which the admin list has no equivalent of.
export type MyWhiteboardListItem = {
  id: string;
  title: string;
  updatedAt: string;
  isOwner: boolean;
...
function NewBoardForm({ onCancel }: { onCancel: () => void }) {
  const [error, formAction, pending] = useActionState(createMyWhiteboardAction, undefined);
  return (
      ...
      <form
        ...
        onSubmit={(e) => {
          ...
          startTransition(async () => {
            await renameMyWhiteboardAction(boardId, value);
            setEditing(false);
            router.refresh();
          });
        }}
  ...
  return (
    <Button
      ...
      onClick={async (e) => {
        ...
        if (!ok) return;
        startTransition(async () => {
          await deleteMyWhiteboardAction(boardId);
          router.refresh();
        });
      }}
...
export function MyWhiteboardList({ items }: { items: MyWhiteboardListItem[] }) {
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((item) => (
            <div key={item.id} className="relative">
              <Link
                href={`/dashboard/whiteboards/${item.id}`}
              ...
              >
                ...
                {!item.isOwner && item.ownerName && (
                  <p className="text-xs text-dark-muted">Chủ sở hữu: {item.ownerName}</p>
                )}
