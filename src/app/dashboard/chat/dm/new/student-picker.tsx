...
import { searchStudentsAction, startDirectThreadAction } from "../../actions";
...
export function StudentPicker() {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      setSearching(true);
      const found = await searchStudentsAction(trimmed);
      if (!cancelled) {
        setResults(found);
        setSearching(false);
      }
  ...
  function handleSelect(studentId: string) {
    ...
    startTransition(async () => {
      const result = await startDirectThreadAction(studentId);
