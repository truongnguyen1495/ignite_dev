...
// gives them one back, same fixed bottom-right/safe-area convention as
// GuestChatWidget (guest/guest-chat-widget.tsx) so it never collides with a
// notch/home-indicator, and same icon as the dashboard header's own cart
// link for a consistent look.
//
// Product pages are public now (guests can view them too — see the
// page-level comment), so this must degrade to "render nothing" for a
// signed-out visitor instead of the old requireActiveStudent(), which would
// hard-redirect anyone without a session straight to /login just for
// viewing the page.
